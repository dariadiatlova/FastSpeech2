...
class Preprocessor:
    ...
    def process_utterance(self, short_name, speaker_idx, filename_idx, include_empty_intervals):
        ...
        if np.isnan(duration).any():
            print(f"{speaker_idx}_{filename_idx} sample duration contains nan")
            return
        dur_filename = f"{speaker_idx}-duration-{filename_idx}.npy"
        np.save(os.path.join(self.out_dir, "duration", dur_filename), duration)

        if np.isnan(pitch).any():
            print(f"{speaker_idx}_{filename_idx} sample pitch contains nan")
        ...
        if np.isnan(energy).any():
            print(f"{speaker_idx}_{filename_idx} sample energy contains nan")
            os.remove(os.path.join(self.out_dir, "duration", dur_filename))
        ...
        if np.isnan(mel_spectrogram).any():
            print(f"{speaker_idx}_{filename_idx} sample mel_spectrogram contains nan")
            os.remove(os.path.join(self.out_dir, "duration", dur_filename))
            os.remove(os.path.join(self.out_dir, "pitch", pitch_filename))
