    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i, tag in enumerate(basenames):
            ...
            self.logger.experiment.log(
                {f"Generated_audio/{tag}": wandb.Audio(
                    synthesized_wav, caption=tag, sample_rate=self.sampling_rate)}
            )
            ...
            if self.global_step == 0:
                ...
                self.logger.experiment.log(
                        {f"Original_audio/{tag}": wandb.Audio(
                            ground_truth_wav, caption=tag, sample_rate=self.sampling_rate)}
                    )

                self.logger.experiment.log(
                    {f"Reconstructed_audio/{tag}": wandb.Audio(
                        vocoder_synthesized_from_gt, caption=tag, sample_rate=self.sampling_rate)}
                )
